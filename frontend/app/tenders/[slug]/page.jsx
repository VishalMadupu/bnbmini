...
import { MapPin, CalendarClock, CalendarDays, Hash, Download, ArrowLeft, Building2, IndianRupee, FileDigit, ExternalLink, Clock, Landmark } from "lucide-react";
    ...
    return (
      <Layout>
        <div className="mx-auto max-w-6xl px-4 py-24 text-center">
          ...
          <Link href="/tenders"><Button className="mt-6 bg-brand-600 text-white hover:bg-brand-700">Browse all tenders</Button></Link>
        </div>
      </Layout>
    );
  }
  ...
  return (
    <Layout>
      ...
      <div className="mx-auto max-w-6xl px-4 py-6 sm:px-6 sm:py-8 lg:px-8">
        <Link href="/tenders" className="inline-flex items-center gap-1 text-sm text-slate-500 hover:text-brand-600">
        ...
        <div className="mt-5">
          ...
          <p className="mt-2 flex items-center gap-2 text-base font-medium text-slate-700 sm:text-lg">
            <Building2 className="h-5 w-5 flex-shrink-0 text-slate-400" /> <span className="break-words">{tender.organization}</span>
          </p>
          {tender.original_reference && (
            <p className="mt-1 break-words font-mono text-sm text-slate-400"># {tender.original_reference}</p>
          )}
        </div>
